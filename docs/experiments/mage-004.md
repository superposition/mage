...
| third rewrite (PR #49 / #52) | 68.62 µs | 8.97 µs | `mage-004` |

All eight values are GPU kernel time from Nsight Systems captures: the sum of captured kernel durations over 100 iterations of the same FP32 shape. The two values in the last row are read from the `mage-004` namespace, which is the evidence for this note; the other five come from the namespaces named beside them. The LayerNorm value at the first rewrite is the warp kernel of PR #42 measured in the later capture session — the `mage-002` namespace records 11.09 µs for the same kernel — and the LayerNorm value at the third rewrite is 8.85 µs in the three other sessions that measured it, against 8.97 µs in the retained capture.
...
## What the mage-004 captures show
...
<figure class="profile-plot">
  <picture>
    <source media="(max-width: 520px)" srcset="{{ '/assets/figures/mage-004/kernel-progression-mobile.svg' | relative_url }}">
    <img src="{{ '/assets/figures/mage-004/kernel-progression.svg' | relative_url }}" width="740" height="510"
  ...
  <figcaption>
    ...
    <div class="profile-links">
      <a href="{{ '/assets/figures/mage-004/kernel-progression.svg' | relative_url }}" download>Download SVG</a>
      <a href="{{ '/assets/figures/mage-004/kernel-progression.png' | relative_url }}" download>PNG</a>
...
<figure class="profile-plot">
  <picture>
    <source media="(max-width: 520px)" srcset="{{ '/assets/figures/mage-004/comparison-views-mobile.svg' | relative_url }}">
    <img src="{{ '/assets/figures/mage-004/comparison-views.svg' | relative_url }}" width="740" height="351"
         alt="GPU kernel time and time around the call per operation in the mage-004 capture: in the kernel panel PyTorch has the shortest time for matrix multiplication and triangle contraction, and Triton for Bias + GELU, LayerNorm and neighbor aggregation; in the span panel Rust has the shortest span for Bias + GELU, LayerNorm and neighbor aggregation, and PyTorch for matrix multiplication and triangle contraction.">
  </picture>
  <figcaption>
    ...
    <div class="profile-links">
      <a href="{{ '/assets/figures/mage-004/comparison-views.svg' | relative_url }}" download>Download SVG</a>
      <a href="{{ '/assets/figures/mage-004/comparison-views.png' | relative_url }}" download>PNG</a>
      <a href="https://github.com/superposition/mage/blob/master/docs/assets/results/mage-004/comparison-profiles.json">Captures ↗</a>
    </div>
    <details class="profile-values">
      <summary>Read the plotted values (µs)</summary>
      <table>
        <caption class="visually-hidden">Rust kernel time and event span per operation in the mage-004 capture</caption>
...
<figure class="profile-plot">
  <picture>
    ...
    <img src="{{ '/assets/figures/mage-004/comparison-kernel.svg' | relative_url }}" width="740" height="650"
         alt="GPU kernel time per operation in the mage-004 capture: PyTorch has the shortest time for matrix multiplication and triangle contraction, and Triton for Bias + GELU, LayerNorm and neighbor aggregation.">
  </picture>
  <figcaption>
...
<figure class="profile-plot">
  <picture>
    <source media="(max-width: 520px)" srcset="{{ '/assets/figures/mage-004/comparison-event-mobile.svg' | relative_url }}">
    <img src="{{ '/assets/figures/mage-004/comparison-event.svg' | relative_url }}" width="740" height="650"
  ...
  <figcaption>
    ...
    <div class="profile-links">
      <a href="{{ '/assets/figures/mage-004/comparison-event.svg' | relative_url }}" download>Download SVG</a>
      <a href="{{ '/assets/figures/mage-004/comparison-event.png' | relative_url }}" download>PNG</a>
      <a href="https://github.com/superposition/mage/blob/master/docs/assets/results/mage-004/comparison-results.json">Samples ↗</a>
    </div>
    <details class="profile-values">
      <summary>Read the plotted values (µs)</summary>
      <table>
...
<figure class="profile-plot">
  <picture>
    <source media="(max-width: 520px)" srcset="{{ '/assets/figures/mage-004/comparison-launches-mobile.svg' | relative_url }}">
    <img src="{{ '/assets/figures/mage-004/comparison-launches.svg' | relative_url }}" width="740" height="650"
  ...
  <figcaption>
    ...
    <div class="profile-links">
      <a href="{{ '/assets/figures/mage-004/comparison-launches.svg' | relative_url }}" download>Download SVG</a>
      <a href="{{ '/assets/figures/mage-004/comparison-launches.png' | relative_url }}" download>PNG</a>
      <a href="https://github.com/superposition/mage/blob/master/docs/assets/results/mage-004/comparison-profiles.json">Captures ↗</a>
    </div>
    <details class="profile-values">
...
| `tiled_matmul_pipeline` (mage-004) | 256 × 256 threads | 96 | 33792 bytes |
| `layer_norm_pair` (mage-003) | 1024 × 256 threads | 39 | 64 bytes |
| `layer_norm_row` (mage-004) | 4096 × 128 threads | 40 | 32 bytes |
...
- **Triton's matrix multiply is not stable between sessions.** In the sessions where the two were captured together it has returned 71.94, 79.84, 86.95 and 90.09 µs, and 100.80 µs in the `mage-004` session, while the Rust kernel has moved only within 68.35–68.68 µs across four sessions. No ranking between the two follows from a single pairing, and the 100.80 µs capture is under load.
...
The harness is unchanged from the earlier notes: `examples/oxide/comparison.py` collects the CUDA-event spans, `profile_suite.py` captures the kernels with Nsight Systems, and `export_comparison.py` writes the retained evidence and rewrites each capture directory relative to the checkout. The run that produced this namespace wrote `artifacts/mage-004-final` and `artifacts/mage-004-nsys`; the published evidence is [comparison-results.json](https://github.com/superposition/mage/blob/master/docs/assets/results/mage-004/comparison-results.json) and [comparison-profiles.json](https://github.com/superposition/mage/blob/master/docs/assets/results/mage-004/comparison-profiles.json).
...
.venv/bin/python examples/oxide/export_comparison.py \
  --events artifacts/mage-004-final --profiles artifacts/mage-004-nsys --experiment mage-004
uv run --script scripts/plot-comparison.py --experiment mage-004
uv run --script scripts/plot-kernel-progression.py --experiment mage-004
```
...
[Measurements and reproduction in GitHub](https://github.com/superposition/mage/blob/master/docs/experiments/mage-004.md) · [The third field note]({{ '/experiments/mage-003/' | relative_url }}) · [The open layer-normalization gap](https://github.com/superposition/mage/issues/54)
